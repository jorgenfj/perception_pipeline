...
DeviceRingBuffer::DeviceRingBuffer(uint32_t slot_count, std::size_t slot_bytes,
                                   ReuseWait reuse_wait, int device_id)
    : reuse_wait_(reuse_wait),
      slot_bytes_(slot_bytes),
      buffers_(slot_count, nullptr),
      data_ready_event_(slot_count, nullptr),
      write_stream_(slot_count, nullptr),
      seq_(slot_count),
  ...
  try {
    for (uint32_t i = 0; i < slot_count; ++i) {
      ...
      cuda_error_check(cudaEventCreateWithFlags(&data_ready_event_[i], cudaEventDisableTiming), "cudaEventCreateWithFlags");
    }
  } catch (...) {
    release();
    throw;
...
uint32_t DeviceRingBuffer::acquire_write_slot(cudaStream_t stream) {
  reject_default_stream(stream, "DeviceRingBuffer::acquire_write_slot");

  const uint32_t slot = next_write_slot_;
  next_write_slot_ = (next_write_slot_ + 1) % slot_count();

  if (reuse_wait_ == ReuseWait::HostSync) {
    cuda_error_check(cudaEventSynchronize(data_ready_event_[slot]), "cudaEventSynchronize");
  } else {
    cuda_error_check(cudaStreamWaitEvent(stream, data_ready_event_[slot], 0),
                     "cudaStreamWaitEvent");
  }

  write_stream_[slot] = stream;
...
void DeviceRingBuffer::mark_slot_written(uint32_t slot, uint64_t timestamp_ns, cudaStream_t stream) {
  if (stream != write_stream_[slot]) {
    throw std::runtime_error(
        "DeviceRingBuffer::mark_slot_written: stream differs from the one acquire_write_slot was "
        "given; the reuse interlock only orders the acquiring stream");
  }

  cuda_error_check(cudaEventRecord(data_ready_event_[slot], stream), "cudaEventRecord");
...
bool DeviceRingBuffer::snapshot_latest(FrameView& out, void* dst, cudaStream_t stream,
  ...
  for (uint32_t attempt = 0; attempt < max_attempts; ++attempt) {
    ...
    cuda_error_check(cudaStreamWaitEvent(stream, view.data_ready_event, 0), "cudaStreamWaitEvent");
    cuda_error_check(cudaMemcpyAsync(dst, view.frame.image_ptr, slot_bytes_, cudaMemcpyDeviceToDevice, stream),
                     "cudaMemcpyAsync");
    cuda_error_check(cudaEventRecord(copied, stream), "cudaEventRecord");
    cuda_error_check(cudaEventSynchronize(copied), "cudaEventSynchronize");
